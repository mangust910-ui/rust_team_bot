...
# Список часовых поясов России (город -> смещение от Москвы)
RUSSIA_TIMEZONES = {
    'Калининград': -1,
    'Москва': 0,
    'Самара': 1,
...
TIMEZONE_NAMES = list(RUSSIA_TIMEZONES.keys())

def format_timezone(city):
    offset = RUSSIA_TIMEZONES[city]
    sign = '+' if offset >= 0 else ''
    return f"{city} (MSK{sign}{offset})"
...
def format_profile(user_id, show_username=False, show_id=False):
    ...
    text += f"🕒 Часов в игре: {u.get('hours', 'Не указано')}\n"
    text += f"⏳ Часов в день: {u.get('hours_per_day', 'Не указано')}\n"
    text += f"⏰ Часовой пояс: {format_timezone(u.get('timezone', 'Москва')) if u.get('timezone') else 'Не указан'}\n"
...
def get_age(message):
    try:
        ...
        save_data()
        # следующий шаг - часы в игре
        kb = choice_keyboard(['0-50', '50-200', '200-500', '500+'])
        msg = bot.send_message(message.chat.id, "🕒 Сколько часов ты наиграл в Rust Mobile? Выбери:", reply_markup=kb)
...
def process_hours(message):
    ...
    save_data()
    # следующий шаг - часов в день
    kb = choice_keyboard(['1-2', '3-4', '5-6', '7+'])
    msg = bot.send_message(message.chat.id, "⏳ Сколько часов в день ты готов играть? Выбери:", reply_markup=kb)
    bot.register_next_step_handler(msg, process_hours_per_day)

def process_hours_per_day(message):
    val = message.text
    if val not in ['1-2', '3-4', '5-6', '7+']:
        bot.send_message(message.chat.id, "❌ Выбери из вариантов.", reply_markup=choice_keyboard(['1-2', '3-4', '5-6', '7+']))
        bot.register_next_step_handler(message, process_hours_per_day)
        return
    user_id = str(message.chat.id)
    data['users'][user_id]['hours_per_day'] = val
    save_data()
    # следующий шаг - часовой пояс (список городов России)
    kb = choice_keyboard(TIMEZONE_NAMES)
    msg = bot.send_message(message.chat.id, "⏰ Выбери свой часовой пояс (по городу):", reply_markup=kb)
    bot.register_next_step_handler(msg, process_timezone)

def process_timezone(message):
    city = message.text
    if city not in TIMEZONE_NAMES:
        bot.send_message(message.chat.id, "❌ Выбери город из списка.", reply_markup=choice_keyboard(TIMEZONE_NAMES))
        bot.register_next_step_handler(message, process_timezone)
        return
    user_id = str(message.chat.id)
    data['users'][user_id]['timezone'] = city
    save_data()
    # следующий шаг - скилл
...
def process_roles(message):
    ...
    save_data()
    # последний шаг - о себе (текстовый ввод)
...
def edit_profile_start(message):
    ...
    kb.add(types.InlineKeyboardButton("Часы в игре", callback_data="edit_hours"))
    kb.add(types.InlineKeyboardButton("Часов в день", callback_data="edit_hours_per_day"))
    kb.add(types.InlineKeyboardButton("Часовой пояс", callback_data="edit_timezone"))
...
def edit_callback(call):
    ...
    if field == 'timezone':
        kb = choice_keyboard(TIMEZONE_NAMES)
        bot.edit_message_text("Выбери новый часовой пояс:", call.message.chat.id, call.message.message_id, reply_markup=kb)
        bot.register_next_step_handler(call.message, lambda m: save_edit(m, field))
    elif field == 'hours_per_day':
        kb = choice_keyboard(['1-2', '3-4', '5-6', '7+'])
        bot.edit_message_text("Выбери новое количество часов в день:", call.message.chat.id, call.message.message_id, reply_markup=kb)
        bot.register_next_step_handler(call.message, lambda m: save_edit(m, field))
    elif field in ['hours', 'skill', 'roles']:
        # для этих полей используем кнопки
        if field == 'hours':
            kb = choice_keyboard(['0-50', '50-200', '200-500', '500+'])
        elif field == 'skill':
            kb = choice_keyboard(['новичок', 'средний', 'опытный', 'профессионал'])
            ...
            kb = choice_keyboard(['комбатер', 'строитель', 'фармер', 'помощник руководителя'])
        bot.edit_message_text(f"Введи новое значение для поля '{field}' (можно выбрать из кнопок):", call.message.chat.id, call.message.message_id, reply_markup=kb)
        bot.register_next_step_handler(call.message, lambda m: save_edit(m, field))
    else:
        # возраст и о себе - текстовый ввод
...
def save_edit(message, field):
    ...
    if field == 'age':
        ...
        if new_val not in ['1-2', '3-4', '5-6', '7+']:
            bot.send_message(message.chat.id, "❌ Выбери: 1-2, 3-4, 5-6, 7+")
            return
    elif field == 'timezone':
        if new_val not in TIMEZONE_NAMES:
            bot.send_message(message.chat.id, "❌ Выбери город из списка.", reply_markup=choice_keyboard(TIMEZONE_NAMES))
            return
    elif field == 'skill':
...
def search_start(message):
    ...
    msg = bot.send_message(message.chat.id, "🕒 Фильтр по общему времени в игре (выбери или пропусти):", reply_markup=kb)
...
def filter_hours(message, filters):
    ...
    if val == '⏭️ Пропустить':
        ...
        filters['hours'] = val
    # Фильтр по часам в день
    kb = choice_keyboard(['1-2', '3-4', '5-6', '7+', '⏭️ Пропустить'], one_time=True)
    msg = bot.send_message(message.chat.id, "⏳ Фильтр по часам в день (выбери или пропусти):", reply_markup=kb)
    bot.register_next_step_handler(msg, filter_hours_per_day, filters)

def filter_hours_per_day(message, filters):
    val = message.text
    if val == '⏭️ Пропустить':
        pass
    elif val not in ['1-2', '3-4', '5-6', '7+']:
        bot.send_message(message.chat.id, "❌ Выбери из вариантов.", reply_markup=choice_keyboard(['1-2', '3-4', '5-6', '7+', '⏭️ Пропустить']))
        bot.register_next_step_handler(message, filter_hours_per_day, filters)
        return
    else:
        filters['hours_per_day'] = val
    # Фильтр по часовому поясу (список городов + пропуск)
    kb = choice_keyboard(TIMEZONE_NAMES + ['⏭️ Пропустить'], one_time=True)
    msg = bot.send_message(message.chat.id, "⏰ Фильтр по часовому поясу (выбери город или пропусти):", reply_markup=kb)
    bot.register_next_step_handler(msg, filter_timezone, filters)

def filter_timezone(message, filters):
    val = message.text
    if val == '⏭️ Пропустить':
        pass
    elif val not in TIMEZONE_NAMES:
        bot.send_message(message.chat.id, "❌ Выбери город из списка.", reply_markup=choice_keyboard(TIMEZONE_NAMES + ['⏭️ Пропустить']))
        bot.register_next_step_handler(message, filter_timezone, filters)
        return
    else:
        filters['timezone'] = val
    # Фильтр по скиллу
...
def filter_skill(message, filters):
    ...
    msg = bot.send_message(message.chat.id, "🎯 Фильтр по ролям (можно несколько через запятую):", reply_markup=kb)
...
def filter_roles(message, filters):
    ...
    if val == '⏭️ Пропустить':
        ...
        if not all(r in allowed for r in roles):
            ...
            return
        filters['roles'] = roles
    # Фильтр по возрасту (кнопки с диапазонами)
    kb = choice_keyboard(['10-15', '16-18', '19-25', '26-35', '36+', '⏭️ Пропустить'], one_time=True)
    msg = bot.send_message(message.chat.id, "📅 Фильтр по возрасту (выбери диапазон или пропусти):", reply_markup=kb)
    bot.register_next_step_handler(msg, filter_age, filters)

def filter_age(message, filters):
    val = message.text
    if val == '⏭️ Пропустить':
        pass
    elif val not in ['10-15', '16-18', '19-25', '26-35', '36+']:
        bot.send_message(message.chat.id, "❌ Выбери диапазон из предложенных.", reply_markup=choice_keyboard(['10-15', '16-18', '19-25', '26-35', '36+', '⏭️ Пропустить']))
        bot.register_next_step_handler(message, filter_age, filters)
        return
    else:
        # преобразуем диапазон в мин и макс
        if val == '10-15':
            filters['age_min'] = 10
            filters['age_max'] = 15
        elif val == '16-18':
    ...
    for uid, u in data['users'].items():
        ...
        if 'hours' in filters and u.get('hours') != filters['hours']:
            match = False
        if 'hours_per_day' in filters and u.get('hours_per_day') != filters['hours_per_day']:
            match = False
        if 'timezone' in filters and u.get('timezone') != filters['timezone']:
            match = False
...
def like_callback(call):
    ...
    if add_like(from_user, to_user):
        ...
        profile_text = format_profile(from_user, show_username=False, show_id=False)
        kb = types.InlineKeyboardMarkup()
...
def admin_all_users(message):
    ...
    for uid, u in data['users'].items():
        ...
        text += f"ID: {uid}, @{username}, {u.get('age', '?')} лет, {u.get('hours', '')} ч, {u.get('hours_per_day', '')} ч/день, {u.get('timezone', '')}\n"
