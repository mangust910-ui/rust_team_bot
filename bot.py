...
# ================== НОВЫЕ ФУНКЦИИ РЕГИСТРАЦИИ С КНОПКАМИ ==================
...
def process_hours(message):
    ...
    if val not in ['0-50', '50-200', '200-500', '500+']:
        bot.send_message(message.chat.id, "❌ Пожалуйста, выбери из предложенных вариантов.", reply_markup=choice_keyboard(['0-50', '50-200', '200-500', '500+']))
...
def process_skill(message):
    ...
    msg = bot.send_message(message.chat.id, "🎯 Выбери свои роли (можно несколько, введи через запятую):\nНажми на кнопку, чтобы выбрать одну, или напиши список.", reply_markup=kb)
...
def process_roles(message):
    ...
    if not all(r in allowed for r in roles):
        bot.send_message(message.chat.id, "❌ Допустимые роли: комбатер, строитель, фармер, помощник руководителя.\nВведи через запятую или выбери одну кнопку.", reply_markup=choice_keyboard(['комбатер', 'строитель', 'фармер', 'помощник руководителя']))
...
def process_playtime(message):
    ...
    if not all(p in allowed for p in parts):
        bot.send_message(message.chat.id, "❌ Допустимые: утро, день, вечер, ночь, будни, выходные.\nВведи через запятую или выбери одну кнопку.", reply_markup=choice_keyboard(['утро', 'день', 'вечер', 'ночь', 'будни', 'выходные']))
...
def like_callback(call):
    ...
    if add_like(from_user, to_user):
        ...
        # Показываем анкету того, кто лайкнул
        profile_text = format_profile(from_user, show_id=is_admin(to_user))
        kb = types.InlineKeyboardMarkup()
        ...
        try:
            bot.send_message(int(to_user), f"🔔 Вас лайкнул {contact_from}!\n\n{profile_text}", reply_markup=kb)
        except:
            pass
        # Проверка взаимности
